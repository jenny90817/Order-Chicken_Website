...
export default function ProductList() {
    ...
    const [products, addProduct, deleteProduct, updateProduct, isLoading, addShoppingcart] = useGetProducts();
    const [newProduct, setNewProduct] = useState<Product>({ id: "", desc: "", price: 0, photo:""});
    ...
    const handleAddButtonClick = async (clickedDrink: Product) => {
      try {
            ...
            <List subheader="" aria-label="product list">
                {products.map((product, index) =>
                    <ListItem divider key={product.desc}>
                        <ListItemText primary={product.desc} secondary={`$${product.price}`} onClick={() => handleClickToShowDialog(product.desc)}></ListItemText>
                    {authContext && getRole === "consumer" &&(
                        <> 
                          <IconButton edge="end" aria-label="add" onClick={() => handleAddButtonClick(product)}>
                            <AddIcon />
                          </IconButton> 
                         </>
                    )} 
                    ...
                    <>
                        ...
                        <IconButton edge="end" aria-label="delete" onClick={() => deleteProduct(product.id,product.photo)}>
                            <DeleteIcon />
                        </IconButton>
                    </>
                    )}
