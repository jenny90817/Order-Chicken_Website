...
import useGetSoups from "./useSoups";
...
export default function SoupList() {
    ...
    const [Soups, addSoups, deleteSoup, updateSoup, isLoading, addShoppingcart] = useGetSoups();
    const [newProduct, setNewProduct] = useState<Soup>({ id: "", desc: "", price: 0, photo: "" });
    ...
    async function addOrUpdate() {
      if (newProduct.id === "") {
          ...
          updateSoup(newProduct);
      }
      setStatus({ ...status, visible: false })
      resetSoup();
  }

    function setUpdateProduct(product: Soup) {
  ...
  const loadProductProfile = async (desc:String) => {
      try {
          const productCollection = collection(db, 'soup');
...
const handleAddButtonClick = async (clickedDrink: Soup) => {
  try {
  ...
  return (
    ...
    {authContext && getRole === "admin" &&(
        ...
        <div>
            ...
            <List subheader="" aria-label="soup list">
                {Soups.map((soup, index) =>
                <ListItem divider key={soup.desc}>
                    <ListItemText primary={soup.desc} secondary={`$${soup.price}`} onClick={() => handleClickToShowDialog(soup.desc)}></ListItemText>
                    {authContext && getRole === "consumer" &&(
                    <> 
                        <IconButton edge="end" aria-label="add" onClick={() => handleAddButtonClick(soup)}>
                            <AddIcon />
                        </IconButton> 
                        </>
                    )}
                        {authContext && getRole === "admin" &&(
                        <> 
                            <IconButton edge="end" aria-label="edit" onClick={() => setUpdateProduct(soup)}>
                                <EditIcon />
                            </IconButton> 
                            <IconButton edge="end" aria-label="delete" onClick={() => deleteSoup(soup.id,soup.photo)}>
                                <DeleteIcon />
                            </IconButton>
                    </>
                    )}
